"use client";

import { Volume2, VolumeX } from "lucide-react";
import { ui } from "@/lib/content";

type MusicToggleProps = {
  visible: boolean;
  playing: boolean;
  onToggle: () => void;
};

export default function MusicToggle({ visible, playing, onToggle }: MusicToggleProps) {
  if (!visible) return null;

  return (
    <button
      type="button"
      onClick={onToggle}
      aria-label={playing ? ui.musicOff : ui.musicOn}
      aria-pressed={playing}
      className="fixed bottom-6 right-6 z-40 flex h-11 w-11 items-center justify-center rounded-full border border-[var(--color-gold)] bg-[var(--color-bg)]/90 text-[var(--color-gold)] shadow-sm backdrop-blur transition-colors hover:bg-[var(--color-gold-soft)]/40"
    >
      {playing ? <Volume2 size={18} /> : <VolumeX size={18} />}
      {playing && (
        <span className="absolute inset-0 animate-ping rounded-full border border-[var(--color-gold)]/40" />
      )}
    </button>
  );
}
